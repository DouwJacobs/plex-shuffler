...
const OLD_LOG_FILE = path.join(__dirname, '../config/logs/plex-shuffler.log');
  ...
  transports: [
    ...
    new winston.transports.DailyRotateFile({
      filename: process.env.CONFIG_DIRECTORY
        ? `${process.env.CONFIG_DIRECTORY}/logs/plex-shuffler-%DATE%.log`
        : path.join(__dirname, '../config/logs/plex-shuffler-%DATE%.log'),
      datePattern: 'YYYY-MM-DD',
      zippedArchive: true,
      maxSize: '20m',
      maxFiles: '7d',
      createSymlink: true,
      symlinkName: 'plex-shuffler.log',
    }),
    new winston.transports.DailyRotateFile({
      filename: process.env.CONFIG_DIRECTORY
